...
def test_console_log_goes_to_tmpdir(tmp_path, vehicle_name):
    ...
    A caller running more than one vehicle, or sharing a machine with
    another user, has to be able to place these logs itself: one fixed
    path cannot serve two writers.
    """
...
def test_console_log_falls_back_to_tmp(vehicle_name):
    """With TMPDIR unset the log still lands in /tmp, as it always has."""
